...
export function SiteFooter() {
  return <footer className="site-footer"><div className="site-shell grid gap-10 py-12 sm:grid-cols-2 lg:grid-cols-[1.2fr_1fr_.8fr_.8fr_.9fr]">
    ...
    <div><h2 className="text-sm font-semibold text-white">Connect</h2><ul className="mt-4 space-y-2 text-sm"><li><Link className="footer-link" href="/contact">Contact the society</Link></li></ul></div>
    <div><h2 className="text-sm font-semibold text-white">Legal</h2><ul className="mt-4 space-y-2 text-sm">{legalLinks.map((item) => <li key={item.href}><Link className="footer-link" href={item.href}>{item.label}</Link></li>)}</ul></div>
